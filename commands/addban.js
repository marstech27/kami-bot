module.exports = async (sock, from, msg, isAdmin, botData, saveBotData, args) => {
    ...
    if (!word) {
        await sock.sendMessage(from, { text: "⚠️ Please specify a word to ban!" });
        return;
    }
    ...
    if (botData.bannedWords[from].includes(word)) {
        await sock.sendMessage(from, { text: `⚠️ \"${word}\" is already banned in this group!` });
        return;
    }
    botData.bannedWords[from].push(word);
    saveBotData();
    await sock.sendMessage(from, { text: `✅ \"${word}\" has been banned in this group!` });
};