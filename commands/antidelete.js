...
async function storeMessage(message) {
    try {
        ...
        // viewOnce messages skip karo - download allowed nahi
        if (message.message?.viewOnceMessage ||
            message.message?.viewOnceMessageV2 ||
            message.message?.viewOnceMessageV2Extension) return;
        ...
        const msg = message.message?.ephemeralMessage?.message || message.message;
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        const isGroup = original.group !== null;
        ...
        if (isGroup) {
            try {
                ...
                groupInfo = `\n*Group:* ${original.group.split('@')[0]}`;
            }
        }

        const infoText = `*Deleted Message*\n*From:* @${senderName}${groupInfo}`;
        await sock.sendMessage(ownerNumber, { text: infoText, mentions: [sender] });

        if (original.content && !original.mediaType) {
            await sock.sendMessage(ownerNumber, { text: original.content });
        }

        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            if (original.mediaType === 'image') await sock.sendMessage(ownerNumber, { image: { url: original.mediaPath } });
            else if (original.mediaType === 'sticker') await sock.sendMessage(ownerNumber, { sticker: { url: original.mediaPath } });
            else if (original.mediaType === 'video') await sock.sendMessage(ownerNumber, { video: { url: original.mediaPath } });
            else if (original.mediaType === 'audio') await sock.sendMessage(ownerNumber, { audio: { url: original.mediaPath }, mimetype: 'audio/mp4' });
